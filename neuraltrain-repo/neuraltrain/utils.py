...
class PackedExperiment(BaseModel):
    """Run several experiments inside one scheduler job."""

    experiments: list[tp.Any]
    n_jobs: int = 1

    _exclude_from_cls_uid: tp.ClassVar[tuple[str, ...]] = ("n_jobs",)
    ...
    def run(self) -> list[tp.Any]:
        ...
        results = Parallel(n_jobs=self.n_jobs)(
            delayed(experiment.run)() for experiment in self.experiments
        )
        return list(results)
...
def pack_experiments_for_submission(
    ...
    ``experiments_per_job`` controls how many experiments go into each scheduler
    job. Use ``-1`` to put all pending experiments in one job. ``n_jobs`` controls
    local parallelism inside each packed job; when left unset it matches
    ``experiments_per_job``.
    """
    if experiments_per_job == 0 or experiments_per_job < -1:
        raise ValueError("experiments_per_job must be positive, or -1 for all.")
    if n_jobs is None:
        n_jobs = experiments_per_job
    if n_jobs == 0:
        raise ValueError("n_jobs must be a non-zero integer.")

    pending = [exp for exp in experiments if _should_submit_experiment(exp)]
    if not pending:
        return []
    ...
    step = len(pending) if experiments_per_job == -1 else experiments_per_job
    return [
        PackedExperiment(
