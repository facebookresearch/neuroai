...
- docs: EEG/EMG Foundation Challenge 2026 starter-kit corrections from participant feedback — every track page documents its exact split and the validation metric that selects the final checkpoint, Track 2 defaults to `Dreyer2023Large` rather than `Stieger2021Continuous`, Track 3's target is the first N2 epoch rather than the first *stable* one and its cohort holds both seen and unseen sleepers at 128 Hz, and the submission page covers registration, the benchopt solver contract, local testing and the upload itself (#277).
