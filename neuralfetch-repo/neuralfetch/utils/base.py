...
def root_study_folder() -> Path:
    """Return the root folder where study data is stored.

    Reads from the ``NEURALSET_STUDY_FOLDER`` environment variable.  Raises
    ``RuntimeError`` if it is not set.  Callers that need a different path
    (e.g. tests using synthetic data) should pass one explicitly rather than
...
def compute_study_info(name: str, folder: str | Path) -> dict[str, tp.Any]:
    ...
    actual: dict[str, tp.Any] = dict(
        ...
        # Coerce to plain Python str so repr() produces 'Ieeg' not np.str_('Ieeg'),
        # which would make the ruff-formatted output unparseable without numpy.
        event_types_in_query={str(t) for t in events["type"].unique()},
    )
    ...
    if isinstance(event, ev.etypes.Fmri):
        ...
        if isinstance(event, tuple(pick_map)):
            requested = pick_map[type(event)]
            if isinstance(requested, str):
                requested = (requested,)
            # Only pick types that are actually labeled in this recording.
            # Raw EDF files loaded without full BIDS sidecar metadata (e.g.
            # Omni-iEEG) may not have channels typed as 'seeg'/'ecog'; in
            # that case we skip the pick and use all channels for data_shape.
            present = set(data.get_channel_types())
            picks = tuple(t for t in requested if t in present)
            if picks:
                data.pick(picks)
...
def format_study_info(actual: dict[str, tp.Any]) -> str:
    ...
    parts = [
        ...
        for f in study.StudyInfo.model_fields
...
def update_source_info(name: str, folder: str | Path | None = None) -> dict[str, tp.Any]:
    ...
    If *folder* is ``None``, uses :func:`root_study_folder` (requires
    ``NEURALSET_STUDY_FOLDER``).  Returns the computed values dict.
